...
- Duplicated: 2457 values. The column is described in the `DATASET_INFO.md` as unique identifier for each booking but there are duplicated values: 
   1. 2430 bookings_id duplicated twice. There is no clear pattern in any of the combinations between cancelled and not_cancelled. 
   2. 27 bookings_id triplicated. There is not enough data nor clear pattern. 
It's necessary to speak to the Ops team about this problems and solve it asap to keep data quality in our database
   
- Observations: the code starts with CNR followed by 7 numbers. IDs do not increment sequentially because they are likely generated by different services (to confirm with Ops team)
...
- Duplicated: 2418 values. The column contains unique customer_id so the analysis assuming that duplicated are the same customer using the service more than one time. The duplicated rows can be divided into:
   1. 2400 customer_id duplicated twice. No clear patterns seen here.
   2. 18 customer_id triplicated. I will leave this analysis for future versions of the project.
