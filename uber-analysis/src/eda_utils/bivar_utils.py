...
def nominal_vs_binary(df, feature_col, target_col, labels,
    ...
    target_label : str, optional
        Human-readable name for the target variable used in chart titles.
        Defaults to *target_col*.
    target_pos_label : str
        Display label for target=1 in residual table. Default ``"Cancelled"``.
    target_neg_label : str
        Display label for target=0 in residual table. Default ``"Not Cancelled"``.
    rate_label : str
        Y-axis label for the rate panel. Default ``"Cancellation Rate"``.
    figsize : tuple
...
def continuous_by_category(df, feature_col, category_col,
    ...
    """Box + violin plots and Kruskal-Wallis / eta-squared for a continuous
    feature grouped by a categorical variable.
    ...
    category_label : str | None
        Human-readable name; defaults to *category_col*.
    figsize : tuple
        Figure size for the two-panel chart.
    cache_path : str | None
        If provided and the file exists, load the image instead of plotting.
        If provided and the file does not exist, save the plot there.

    Returns
    -------
    dict with keys ``kruskal_H``, ``kruskal_p``, ``eta_squared``, ``group_stats``.
    """
    import os
...
def categorical_vs_categorical(df, col_a, col_b,
                                ...
                                cache_prefix=None):
    """Chi-square / Cramér's V analysis with proportions and residuals heatmaps
    ...
    col_a_label, col_b_label : str | None
        Human-readable labels; default to column names.
    top_n : int
        Number of top categories (by volume) of *col_b* to show in heatmaps.
    heatmap_figsize : tuple
        Size of each heatmap figure.
    cache_prefix : str | None
        If given, used as the base path for caching plots.  Two files are
        created: ``<prefix>_proportions.png`` and ``<prefix>_residuals.png``.

    Returns
    -------
    dict with keys ``chi2``, ``p``, ``dof``, ``cramers_v``,
    ``std_residuals``, ``noteworthy_count``.
    """
    import os
